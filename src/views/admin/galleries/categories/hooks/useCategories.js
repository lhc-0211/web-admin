...
import { useGalleryCategoriesStore } from '../store/useGalleryCategoriesStore'
...
export default function useCategories() {
    const {
        ...
        setFilterData,
    } = useGalleryCategoriesStore((state) => state)
    ...
    const swrKey = React.useMemo(() => {
        return ['/api/admin/gallery-categories', params]
    }, [params])

    const { data, error, isLoading, mutate } = useSWR(
        swrKey,
        ([_, p]) => apiGetGalleryCategoriesAdmin(p), // Truyền params vào API
        {
