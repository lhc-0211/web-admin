...
import { apiCreateDocumentTypeAdmin } from '@/services/DocumentsService' // Đổi tên API phù hợp
...
const TypesCreateModal = ({ isOpen, onClose }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            await apiCreateDocumentTypeAdmin(body)

            mutate() // Refresh danh sách loại tài liệu
            toast.push(
                <Notification title="Thành công" type="success">
                    Tạo loại tài liệu <strong>{data.name}</strong> thành công!
                </Notification>,
            )

            handleClose()
        } catch (error) {
            console.error('Lỗi tạo loại tài liệu:', error)
            toast.push(
                <Notification title="Lỗi" type="danger">
                    Tạo loại tài liệu thất bại:{' '}
                        ...
                        >
                            <Controller
                                ...
                                render={({ field }) => (
                                    <>
                                        <Input
                                            placeholder="Ví dụ: CONTRACT, DECISION"
                                            {...field}
                                            invalid={!!errors.code}
                                            onChange={(e) =>
                                                field.onChange(
                        ...
                        >
                            <Controller
                                ...
                                render={({ field }) => (
                                    <>
                                        <Input
                                            placeholder="Ví dụ: Hợp đồng, Quyết định, Báo cáo"
                                            {...field}
                                            invalid={!!errors.name}
                                        />
                                        {errors.name && (
...
export default TypesCreateModal
