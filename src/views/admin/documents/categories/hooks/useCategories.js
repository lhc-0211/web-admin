...
import { useDocumentsCategoriesStore } from '../store/useDocumentsCategoriesStore'
...
export default function useCategories() {
    const {
        ...
        setFilterData,
    } = useDocumentsCategoriesStore((state) => state)
    ...
    const swrKey = React.useMemo(() => {
        return ['/api/admin/document-categories', params]
    }, [params])

    const { data, error, isLoading, mutate } = useSWR(
        swrKey,
        ([_, p]) => apiGetDocumentCategoriesAdmin(p), // Truyền params vào API
        {
